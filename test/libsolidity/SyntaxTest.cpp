...
void SyntaxTest::printContract(std::ostream& _stream, std::string const& _linePrefix, bool const _formatted) const
{
	if (_formatted)
	{
		if (m_source.empty())
		...
		std::vector<char const*> sourceFormatting(m_source.length(), formatting::RESET);
